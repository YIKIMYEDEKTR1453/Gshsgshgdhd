...
def lo(word):
    heron = [
        "[\x1b[1;91m■\x1b[0m□□□□□□□□□]", "[\x1b[1;92m■■\x1b[0m□□□□□□□□]",
        "[\x1b[1;93m■■■\x1b[0m□□□□□□□]", "[\x1b[1;94m■■■■\x1b[0m□□□□□□]",
        "[\x1b[1;95m■■■■■\x1b[0m□□□□□]", "[\x1b[1;96m■■■■■■\x1b[0m□□□□]",
        "[\x1b[1;97m■■■■■■■\x1b[0m□□□]", "[\x1b[1;97m■■■■■■■■\x1b[0m□□]",
        "[\x1b[1;97m■■■■■■■■■\x1b[0m□]", "[\x1b[1;97m■■■■■■■■■■\x1b[0m]"
    ]
    for _ in range(3):
        for bar in heron:
            sys.stdout.write(f'\r{word} {bar}')
            time.sleep(0.03)
            sys.stdout.flush()

def follow_challenge(session, url, headers):
    while True:
        response = session.get(url, headers=headers)
        html = response.text
        match = re.findall(r'toNumbers\("([a-fA-F0-9]+)"\)', html)
        ...
        loc = re.search(r'location\.href="([^"]+)"', html)
        if loc:
            next_url = loc.group(1)
            url = next_url
            continue
        break
    return response
...
def check_tuk_api():
    ekran_temizle()
    lo(f"{yıkım_M} TUK API'ye bağlanılıyor ...")
    print("\n")

    session = requests.Session()
    base_url = "https://vipadmin.ct.ws/usbdusvsujsgYsj.php"
    api_param = "instagram_vip_tool_v9_1"
    client_id = "0b58af4bb10b6045dfb604b35ab3311a"
    timestamp = int(time.time())
    secret = "YIKIMTOOLCRATE"
    signature = hashlib.sha256(f"{client_id}{timestamp}{secret}".encode()).hexdigest()
    ...
    headers = {
        ...
        'Referer': f'{base_url}?api={api_param}&client_id={client_id}&timestamp={timestamp}&signature={signature}',
    ...
    url = f"{base_url}?api={api_param}&client_id={client_id}&timestamp={timestamp}&signature={signature}"
    response = follow_challenge(session, url, headers)
    ...
    try:
        ...
        print(yıkım_F + "■" * 67)
        print("\n")
        print(f"{yıkım_X} TUK API'den Gelen Yanıt:\n")
        print(f"{yıkım_C}Status   : {data.get('status', '')}")
        print(f"Message  : {data.get('message', '')}")
        print(f"İletişim : {data.get('iletisim', '')}")
        print(yıkım_F + "■" * 67 + "\n")

        status = data.get('status', '').lower()

        if status == "aktif":
            print(yıkım_F + " TUK API aktif, devam ediliyor...\n")
            time.sleep(0.8)
            ekran_temizle()
            os.system("clear")

        elif status == "kapali" or status == "offline":
            print(yıkım_Z + " TUK API şu anda kapalı, işlem sonlandırılıyor.\n")
            sys.exit()

        else:
            print(yıkım_X + " Beklenmeyen bir yanıt geldi: " + yıkım_C + str(data))
            sys.exit()

    except requests.exceptions.RequestException as e:
        print(yıkım_Z + " TUK API'ye bağlanırken bir hata oluştu:", e)
        sys.exit()

    except ValueError:
        print(yıkım_Z + " TUK API'den gelen veri JSON değil ya da hatalı.")
        sys.exit()
